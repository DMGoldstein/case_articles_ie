...
// space and time), and complete mediation: case inventory
// affects indefinite articles only through definite articles.
// No explicit phylogenetic random effect is included.
//
...
// an intercept, the cross-link from def, and the spatiotemporal
// latent field.
//
...
model {
  ...
  // Likelihood (no phylogenetic term; no direct case effect on indef)
  for (n in 1:N) {
    // Case -> Definite Article
...
generated quantities {
...

  {
    ...
    // p_indef_avg[k] is not constant across k here because the loop averages over
