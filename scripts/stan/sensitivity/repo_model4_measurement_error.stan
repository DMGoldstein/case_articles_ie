// ============================================================
// Measurement-error sensitivity model (primary specification)
//
// This is a variant of Model 4 (repo_model4_phylo_stgp.stan)
// used to assess how sensitive the main results are to
// uncertainty in the recorded case inventory counts.
//
// The measurement-error mechanism replaces the hard assignment
// of each language to its observed case level n_cases[i] with
// a soft, weighted average over all K levels. The weights
// follow a discrete Gaussian centered on the observed level:
//
//   p_i[k] ∝ exp(-0.5 * ((k - n_cases[i]) / sigma_cases)^2)
//
// The expected ordinal case effect for language i is then:
//
//   g_def_exp[i]   = sum_k p_i[k] * g_def[k]
//   g_indef_exp[i] = sum_k p_i[k] * g_indef[k]
//
// When sigma_cases = 0, the weights collapse to a point mass
// and the model is identical to Model 4. As sigma_cases
// increases, probability mass spreads to adjacent case levels.
// This spread reflects growing uncertainty in the true case count.
//
// sigma_cases is passed as a fixed data value so that the
// model can be run across a grid of values without
// recompilation. See run_measurement_error.R for the grid
// values used in the analysis (Section S3).
//
// All other modeling choices (priors, kernel, phylogenetic
// mixture, mediation structure) are identical to Model 4.
//
...
data {
  ...
  // Measurement-error spread on n_cases
  // Set to 0 to recover the hard-assignment behavior of Model 4
  // Typical grid: c(0.00, 0.25, 0.50, 0.75, 1.00, 1.50, 2.00, 3.00)
...
transformed parameters {
  ...
  // Measurement-error soft assignment over case levels
  // For each language, replace the hard case index with a weighted
  // expectation over all K levels. When sigma_cases < 1e-12 the
  // weights collapse to a point mass (identical to Model 4).
  vector[N] g_def_exp;
  vector[N] g_indef_exp;
  {
    if (sigma_cases < 1e-12) {
      // Hard assignment: recovers Model 4 exactly
      for (n in 1:N) {
        g_def_exp[n]   = g_def[n_cases[n]];
        g_indef_exp[n] = g_indef[n_cases[n]];
      }
    } else {
      // Soft assignment: discrete Gaussian weights centered on observed level
      for (n in 1:N) {
        vector[K] logw;
        for (k in 1:K) {
          real d  = k - n_cases[n];
          logw[k] = -0.5 * square(d / sigma_cases);
        }
        vector[K] p = softmax(logw);
        g_def_exp[n]   = dot_product(p, g_def);
        g_indef_exp[n] = dot_product(p, g_indef);
      }
    }
  }
}
...
model {
  ...
  // Likelihood: uses ME-adjusted expected ordinal effects
  for (n in 1:N) {
